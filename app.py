...
def set_theme():
    ...
    <style>
        ...
        .stTextInput label, .stNumberInput label, .stSelectbox label, .stDateInput label { color: #FFFFFF !important; }
...
def calcular_valor_presente(valor_futuro, taxa_diaria, dias):
    ...
    except: return float(valor_futuro)
...
def main():
    ...
    with st.form("simulador_form"):
        ...
        with col1:
            ...
            taxa_mensal = st.number_input("Taxa de Juros Mensal (%)", value=st.session_state.taxa_mensal, step=0.01, format="%.2f", key="taxa_mensal_input")
            modalidade = st.selectbox("Modalidade de Pagamento", ["mensal", "mensal + balão", "só balão anual", "só balão semestral"], key="modalidade")
            
            tipo_balao, agendamento_baloes = "anual", "Padrão" # Defaults
            if modalidade == "mensal + balão":
                ...
                max_parcelas = st.session_state.get('qtd_parcelas', 180) # Usa o valor atual do input de parcelas
                if not isinstance(max_parcelas, int) or max_parcelas <= 0: max_parcelas = 1

                if agendamento_baloes == "Personalizado (Mês a Mês)":
                    st.multiselect("Selecione os meses dos balões:", options=list(range(1, max_parcelas + 1)), key="meses_baloes")
                elif agendamento_baloes == "A partir do 1º Vencimento":
                    st.number_input("Mês de Vencimento do 1º Balão", min_value=1, max_value=max_parcelas, value=12, step=1, key="mes_primeiro_balao")
        
        with col2:
            qtd_parcelas = st.number_input("Quantidade de Parcelas", min_value=0, max_value=360, step=1, key="qtd_parcelas")
            valor_parcela = st.number_input("Valor da Parcela (R$)", help="Deixe 0 para cálculo automático, exceto no modo 'mensal + balão'.", min_value=0.0, step=100.0, format="%.2f", key="valor_parcela")
            valor_balao = 0.0
            if "balão" in modalidade:
                valor_balao = st.number_input("Valor do Balão (R$)", help="Deixe 0 para cálculo automático, exceto no modo 'mensal + balão'.", min_value=0.0, step=1000.0, format="%.2f", key="valor_balao")
    ...
    if submitted:
        try:
            # Atualiza a taxa de juros no estado para persistência
            st.session_state.taxa_mensal = st.session_state.taxa_mensal_input
            
            # Condição de juros zero
            ...
            # Lógica para determinar qtd_baloes e parâmetros
            meses_baloes, mes_primeiro_balao = [], 12
            if modalidade == "mensal + balão":
                if agendamento_baloes == "Personalizado (Mês a Mês)":
            ...
            elif "balão" in modalidade:
                qtd_baloes = atualizar_baloes(modalidade, qtd_parcelas, tipo_balao if 'tipo_balao' in locals() else None)

            # Lógica de cálculo dos valores das parcelas/balões
            valor_parcela_final, valor_balao_final = 0.0, 0.0
            v_primeira_p, v_primeiro_b = None, None
            ...
            if taxa_mensal_para_calculo == 0.0:
                ...
                if qtd_baloes > 0:
                    if modalidade in ["só balão anual", "só balão semestral"]:
                        ...
                        datas_b = [ajustar_data_vencimento(data_entrada, periodo, i, dia_vencimento) for i in range(1, qtd_baloes + 1)]
                    elif modalidade == "mensal + balão":
                        if agendamento_baloes == "Personalizado (Mês a Mês)": datas_b = [ajustar_data_vencimento(data_entrada, "mensal", m, dia_vencimento) for m in meses_baloes]
                        elif agendamento_baloes == "A partir do 1º Vencimento":
                            base = ajustar_data_vencimento(data_entrada, "mensal", mes_primeiro_balao, dia_vencimento)
                            datas_b = [ajustar_data_vencimento(base, tipo_balao, i) for i in range(qtd_baloes)]
                        else:
                            intervalo = 12 if tipo_balao == 'anual' else 6
                            datas_b = [ajustar_data_vencimento(data_entrada, "mensal", i * intervalo, dia_vencimento) for i in range(1, qtd_baloes + 1)]
            ...
            cronograma = gerar_cronograma(valor_financiado, valor_parcela_final, valor_balao_final, qtd_parcelas, qtd_baloes, modalidade, tipo_balao, data_entrada, taxas, valor_primeira_parcela=v_primeira_p, valor_primeiro_balao=v_primeiro_b, agendamento_baloes=agendamento_baloes, meses_baloes=meses_baloes, mes_primeiro_balao=mes_primeiro_balao)
            ...
            if valor_parcela_final > 0: c3.metric("Valor da Parcela", formatar_moeda(valor_parcela_final))
            if valor_balao_final > 0: c4.metric("Valor do Balão", formatar_moeda(valor_balao_final))
